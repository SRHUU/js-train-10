...
function customAssign(...objects) {
  ...
  for (const key in newObject) {
    if (typeof newObject[key] === "number" && newObject[key] !== null) {
...
function customObjectFromEntries(entries) {
  ...
  const layout = entries.map(([key, value]) => {
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  const count = Object.assign(obj1, obj2);
  const freeze = Object.freeze(count);
  if (Object.isFrozen(freeze) === true) {
    return freeze;
  } else {
    return "Eror, the object was not frozen";
  }
}
...
function sealAndCheck(obj) {
  ...
  return Object.isSealed(Object.seal(obj));
}
...
function checkOwnership(obj1, obj2, key) {
  ...
  const element1 = obj1.hasOwnProperty(key);
  const element2 = obj2.hasOwnProperty(key);
  if (Object.is(element1, element2) === true) {
    return true;
  } else {
    return false;
  }
...
function getObjectValuesSum(obj) {
  ...
  const sum = newValues.reduce((num, el) => num + el, 0);
  return sum;
}
